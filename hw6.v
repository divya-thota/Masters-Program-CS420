...
Proof.
intros.
  apply Examples.xyz_rw_ex in H1.
  - destruct H1; destruct H1; destruct H4.
    destruct H5; destruct H6.
    subst.
    rewrite H5,H6 in H2, H3. 
    exists (length x); exists (length y); exists x0.
    split.
    -- unfold not in *.
...
Proof.
  ...
  unfold  L5 in N.
  unfold In in N.
  destruct N as (w1, (w2, Ha)).
  simpl in *.
  rewrite app_nil_r in Ha.
  apply l5_not_regular_4 with (p:=p) in Ha.
  - assumption.
  - auto.
  -auto.
  -auto.
